...
string GetBookTitle(string path){
    if(Books[path] && Books[path]["title"])
        return Books[path]["title"];
    return "Untitled";
}
...
mixed *LoadChapters(string Source){
    ...
    if (!Books[Source]["items"])
        Books[Source]["items"] = ([]);
    if(!Books[Source]["reads"])
        Books[Source]["reads"] = ([]);
    ...
    foreach (string chap in get_dir(Source + "/"))
    {
        ...
        if (sizeof(statinfo) != 4
            || !intp(statinfo[1])
            || (Books[Source][this_path] && Books[Source][this_path] == statinfo[1])
        ){
            continue;
        }
        else {

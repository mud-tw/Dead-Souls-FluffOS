...
varargs void tc(mixed str, string col, object dude){
    string prefix;
    if(! stringp(str)) str = sprintf("%O", str);
    if (!col) col = "magenta";
...
varargs void tell_env(mixed str, mixed exclude, mixed env){
    if (functionp(str))
        str = unguarded(str);
    if (!stringp(str)) return;
    if(stringp(env) && catch(env = load_object(env))) return;

    if (!env)
        environment(this_player() || this_object())->eventPrint(str, MSG_ENV, exclude);
    else
        env->eventPrint(str, MSG_ENV, exclude);
}
